...
export async function GET(req: Request) {
  try {
    ...
    // Попытка распарсить JSON — если Bitrix вернул ошибку HTML / текст, вернём это клиенту
    try {
      ...
      // не JSON — вернём как текст для диагностики
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.error("Bitrix createDeal non-JSON:", text);
      return NextResponse.json({ error: "bitrix_non_json_response", body: text }, { status: 502 });
    }
  } catch (err) {
